...
class CVE_Scanner_Main:
    ...
    def run_all_scans(self, ports):
        ...
        try:
            if ['443', '80'] in ports:
                shellshock_scanner = ShellshockScanner(f"{http_url}/cgi-bin/test.cgi")
                results['Shellshock'] = shellshock_scanner.scan_for_shellshock()

                struts_scanner = ApacheStrutsScanner(f"{http_url}/showcase.action")
                results['ApacheStruts'] = struts_scanner.scan_for_apache_struts()

                struts2_scanner = StrutsScanner(http_url)
                ...
                if ['443'] in ports:
                    ...
                    cve20200688_scanner = CVE20200688Tester(https_url)
